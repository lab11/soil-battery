...
def load_and_process_data(args):
        ...
        data_df = None
        new_datasets = glob("data/labincubate/rocket4/*-f29.csv")
...
def _plot_and_save_data(args, data_df):
        ...
        start_time = data_df.index[0].date()
        end_time = data_df.index[-1].date()
        
        # setup some paths
        fig_dir = os.path.join(args.dd, "figs")
        #fig_file = os.path.join(fig_dir, "sensor-{}_from-{}_to-{}.{}".format(int(sensor_id), start_time, end_time, args.ft))
        fig_file = os.path.join("", "sensor-{}_from-{}_to-{}.{}".format(int(sensor_id), start_time, end_time, args.ft))
        # make the figures directory if it doesn't already exist
        os.makedirs(fig_dir, exist_ok=True)

        # calculate the rolling average over a 5 minute window
        rollBoi = data_df.rolling(5*6).mean()
        ...
        ax1.set_ylabel('VWC (%)')
        ax1.plot(rollBoi.index, rollBoi['VWC'], color=VWC_color, ls=VWC_style)
        ax1.tick_params(axis='y', labelcolor=VWC_color)
        ax1.set_ylim(0, 0.6)

        ax2 = ax1.twinx()
        ax2.set_ylabel('EC (mS/m)')
        ax2.plot(rollBoi.index, rollBoi['EC'], color=EC_color, ls=EC_style)
        ...
        ax1.grid(True)
        ax1.legend(['Volumetric Water Content (VWC)'], loc='lower left', prop={'size': 6})
        ax2.legend(['Electrical Conductivity (EC)'], loc='lower right' , prop={'size': 6})
        ...
        ax3.grid(True)
        ax3.set_ylim(0,30)
        ax3.plot(rollBoi.index, rollBoi['temp'], color=temp_color, ls=temp_style)
...
if __name__ == "__main__":
        ...
        parser.add_argument('-ft', type=str, default="pdf",
                                                help="figure type (default is 'pdf')")
