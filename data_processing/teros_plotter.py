...
def _plot_and_save_data(args, data_df):
	...
	plt.suptitle("Sensor {} data from {} to {}".format(int(sensor_id), start_time, end_time))

	plt.subplot(311)
	#plt.xlabel("Time")
	plt.ylabel("Temperature (C)")
	plt.ylim(0, 30)
	plt.plot(rollBoi.index, rollBoi['temp'])
	
	plt.subplot(312)
	#plt.xlabel("Time")
	plt.ylabel("Electrical Conductivity")
	plt.ylim(0, 600)
	plt.plot(rollBoi.index, rollBoi['EC'])
	
	plt.subplot(313)
	plt.xlabel("Time")
	plt.ylabel("Volumetric Moisture Content")
	plt.ylim(0, 0.5)
	plt.plot(rollBoi.index, rollBoi['VWC'])
